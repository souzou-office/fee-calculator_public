...
function Card({item,index,onUpdate,onRemove,g,scTotal=0}){
  ...
  const printSheet=()=>{
    ...
    const html=`<!doctype html><html lang=ja><head><meta charset=utf-8><title>登録免許税計算シート</title><style>*{box-sizing:border-box}body{font-family:'Hiragino Kaku Gothic ProN','Noto Sans JP',sans-serif;color:#1a2233;margin:28px;font-size:13px}h1{font-size:17px;text-align:center;letter-spacing:.18em;margin:0 0 4px}.date{text-align:right;color:#555;font-size:12px;margin-bottom:14px}h2{font-size:13px;background:#eef2ff;padding:5px 9px;border-left:4px solid #4338ca;margin:16px 0 6px}table{width:100%;border-collapse:collapse;margin-bottom:6px}td,th{border:1px solid #d7dbe3;padding:5px 8px}th{background:#f4f6fb;font-weight:600;font-size:12px}.r{text-align:right;font-variant-numeric:tabular-nums}.c{text-align:center}.sum{display:flex;justify-content:space-between;padding:3px 9px}.sum b{font-variant-numeric:tabular-nums}.tot{margin-top:10px;padding:9px 12px;border:2px solid #4338ca;border-radius:6px;display:flex;justify-content:space-between;font-size:15px;font-weight:700;color:#4338ca}.note{color:#888;font-size:11px;margin-top:8px}@media print{body{margin:14mm}}</style></head><body><h1>登録免許税計算シート</h1><h2>土地${parcels.length?"（敷地権）　一筆の価格 ÷ 持分分母 × 持分分子":""}</h2>${parcels.length?`<table><thead><tr><th>区分</th><th>一筆の価格</th><th>持分(分子/分母)</th><th>敷地評価額</th></tr></thead><tbody>${landRows}</tbody></table>`:""}<div class=sum><span>評価額合計</span><b>${yen(landSum)}</b></div><div class=sum><span>課税標準①（千円未満切捨）</span><b>${lb.toLocaleString()}</b></div><div class=sum><span>土地の登録免許税　①×${lr*1000}/1000　②</span><b>${yen(lt)}</b></div><h2>建物</h2><div class=sum><span>評価額合計</span><b>${yen(braw)}</b></div><div class=sum><span>課税標準③（千円未満切捨）</span><b>${bb.toLocaleString()}</b></div><div class=sum><span>建物の登録免許税　③×${br*1000}/1000　④</span><b>${yen(bt)}</b></div><div class=tot><span>登録免許税合計（②＋④・百円未満切捨）</span><span>${yen(total)}</span></div><script>window.onload=function(){setTimeout(function(){window.print()},250)}</script></body></html>`;
